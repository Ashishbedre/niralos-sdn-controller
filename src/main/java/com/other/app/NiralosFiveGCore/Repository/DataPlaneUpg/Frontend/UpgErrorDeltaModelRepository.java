...
public interface UpgErrorDeltaModelRepository extends JpaRepository<UpgErrorDeltaModel, Long>{
	...
	@Query(value = "UPDATE upg_errordelta_data m SET m.nat44_ed_out2in_slowpath_no_translation=?1,"
			...
			+ "m.MEC_tx_Tx_packet_drops__dpdk_tx_failure=?38,m.thread=?42 WHERE m.tenent_id=?40 AND m.site_id=?39 AND m.agent_id = ?41", nativeQuery = true)
	public void updateError(Integer nat44_ed_out2in_slowpath_no_translation, int dpdk_input_no_error,
			...
			int mEC_tx_DPDK_tx_function_returned_an_error, int mEC_tx_Tx_packet_drops__dpdk_tx_failure, String siteId, String tenentId, String agentId, 
			Long thread, String nfName, String nfType);

	@Query(value="SELECT COUNT(*) FROM upg_errordelta_data u WHERE u.tenent_id = ?1 AND u.site_id = ?2 AND u.agent_id = ?3",nativeQuery=true)
	...
	@Query("SELECT COUNT(DISTINCT l.siteId) FROM UpgErrorDeltaModel l WHERE tenentId =?1")
	public Integer countofSite(String tenentId);
	...
	@Query(value = "SELECT tb.id,tb.thread- ts.thread AS thread ,"
			...
			+ "		tb.n3_gnb_output_interface_is_deleted- ts.n3_gnb_output_interface_is_deleted n3_gnb_output_interface_is_deleted ,\r\n"
			...
			+ "		JOIN  upg_error_data tb ON ts.id = tb.id WHERE ts.tenent_id = ?1 AND ts.site_id = ?2 AND ts.nf_name=?3 AND ts.nf_type=?4", nativeQuery = true)
	public List<UpgErrorDeltaModel> subtractDaata(String tenentId, String siteId, String nfName, String nfType);
