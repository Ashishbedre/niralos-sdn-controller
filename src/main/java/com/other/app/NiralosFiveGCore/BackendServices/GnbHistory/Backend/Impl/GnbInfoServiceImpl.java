...
public class GnbInfoServiceImpl implements GnbInfoService {
    ...
    private static final Logger loggerTypeA = LoggerFactory.getLogger("5G-BACKEND:GnbInfoServiceImpl");
...
//		internalDataFrontendService.fetchDeployedItemInformation().block();
    	...
    	for(int count1 = 1;count1<=Integer.parseInt(amfIp);count1++) {
            ...
            loggerTypeA.info("fetchDataAndSaveToDatabase: Processing network function {}", networkFunctionName);
            try {
                    ...
                    if (response.getGnbList() != null) {
                        for (GnbListDTO g1 : response.getGnbList()) {
                            ...
                            if (count == 0) {
                                ...
                                loggerTypeA.info("fetchDataAndSaveToDatabase: Saved new GnbInfo");
        	...
        	loggerTypeA.error("fetchDataAndSaveToDatabase: Unable to fetch Gnb Information from 5GCore");
        }
    }
        }
}
