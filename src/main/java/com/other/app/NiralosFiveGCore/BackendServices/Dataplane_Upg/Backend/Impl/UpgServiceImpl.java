...
public class UpgServiceImpl implements UpgService {
	...
	private static final Logger loggerTypeA = LoggerFactory.getLogger("5G-BACKEND:UpgServiceImpl");
...
//				.defaultHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE).build();
		    ...
		    loggerTypeA.info("saveUpgData: Initiating data retrieval for UPF instances.");
//			if (siteId != null && tenantId != null) {

				for (int count = 1; count <= Integer.parseInt(upfIp); count++) {
					loggerTypeA.info("saveUpgData: Processing UPF instance with upfIp count");
					...
					try {
						...
						for (UpgStatModelDto statsModel : data) {
							if (upgRepository.countInterfaceName(statsModel.getInterfaceName(), networkFunctionName, nfServiceImpl.nfTypeofUpf) == 0) {
								...
								loggerTypeA.warn("saveUpgData: In upgRepository countInterfaceName value is not Zero");
						...
						loggerTypeA.error("Unable to fetch UPGStat data from Dataplane");
					}
				}
