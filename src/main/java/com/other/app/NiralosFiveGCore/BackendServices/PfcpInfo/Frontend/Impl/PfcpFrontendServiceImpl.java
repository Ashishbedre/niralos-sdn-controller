...
public class PfcpFrontendServiceImpl implements PfcpFrontendService {
	private static final Logger loggerTypeB = LoggerFactory.getLogger("5G-FRONTEND:PfcpFrontendServiceImpl");
	//final Logger logger = LoggerFactory.getLogger(PfcpFrontendServiceImpl.class);
	 ...
	 @Autowired
	    ...
	    public PfcpInfoRootFrontendDto getPfcpInfoRootDtoByTenantAndSite() {
	        try {
				loggerTypeB.info("PFCP: Fetching PFCP info from the database.");
	            ...
	            if (pfcpInfoList.isEmpty()) {
					loggerTypeB.info("PFCP: No PFCP info found in the database.");
	                return null;
	            }
				...
				loggerTypeB.info("PFCP: Successfully mapped PFCP info.");

	            return pfcpInfoDto;
	        } catch (Exception e) {
	            loggerTypeB.error("Error fetching PFCP data from the database.", e);
	        }
	        return null;
	    }
	    ...
	    public PfcpInfoFrontendDto getFailReasonList() {
			...
			loggerTypeB.info("PFCP: Successfully fetched and mapped PFCP fail reasons.");
	        ...
	        return dto;
	    		...
	    		loggerTypeB.error("PFCP : Not much enough Data to show");
			}
			return null;
	    }
}
