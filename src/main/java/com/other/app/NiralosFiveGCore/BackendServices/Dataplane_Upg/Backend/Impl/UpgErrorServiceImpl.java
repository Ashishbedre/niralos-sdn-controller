	...
	public void saveUpgErrorData() {
		...
		try {
			...
			for (UpgUpfDto upgErrorModel : data) {
				...
				if(upgErrorRepository.countErrorData(LocalSdnId,tenantId,siteId)==0)
				{
				...
				if(upfErrorRepository.countErrorData(LocalSdnId,tenantId,siteId)==0) {
					...
					upfErrorModel.setAgentId(LocalSdnId);
					upfErrorModel.setTenentId(tenantId);
					upfErrorModel.setSiteId(siteId);
					...
					upfErrorRepository.updateError(upgErrorModel.getTun_open_failure_upf(),upgErrorModel.getInvalid_eth_type_upf(),
							...
							,LocalSdnId, tenantId, siteId,networkFunctionName,nfServiceImpl.nfTypeofUpf);
				}
