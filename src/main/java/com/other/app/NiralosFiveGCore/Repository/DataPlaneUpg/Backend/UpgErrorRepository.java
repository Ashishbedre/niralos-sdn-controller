...
public interface UpgErrorRepository extends JpaRepository<UpgErrorModel, Long>{
	...
	@Query("SELECT COUNT(u) FROM UpgErrorModel u WHERE u.agentId = ?1 AND u.tenentId = ?2 AND u.siteId = ?3")
	...
	@Query(value = "UPDATE upg_error_data m SET m.nat44_ed_out2in_slowpath_no_translation=?1,"
			...
			+ "m.MEC_tx_Tx_packet_drops__dpdk_tx_failure=?38,m.agent_id=?40, m.thread=?39 WHERE m.tenent_id=?41 "
			+ "AND m.site_id=?42 AND m.nf_name=?43 AND m.nf_type=?44", nativeQuery = true)
	public void updateError(Integer nat44_ed_out2in_slowpath_no_translation,Integer dpdk_input_no_error,
			...
			,String agentId, String tenentId,
			String siteId,
			String nfName,
			String nfType);
	...
	@Query(value="SELECT * FROM upg_error_data upg",nativeQuery = true)
	...
	@Query("SELECT COUNT(DISTINCT l.siteId) FROM UpgErrorModel l WHERE tenentId =?1")
	...
	@Query(value = "SELECT tb.id,tb.thread- ts.thread AS thread ,"
			...
			+ "		0 - ts.n3_gnb_output_interface_is_deleted n3_gnb_output_interface_is_deleted ,\r\n"
			...
			+ "		JOIN  upg_errordelta_data tb ON ts.id = tb.id WHERE ts.tenent_id = ?1 AND ts.site_id = ?2 "
			+ " 	AND ts.nf_name=?3 AND ts.nf_type=?4", nativeQuery = true)
	public List<UpgErrorModel> subtractFromZero(String tenentId, String siteId, String nfName, String nfType);
