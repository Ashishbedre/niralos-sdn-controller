...
//	Select List of Alarms from database
	@Query(value = "SELECT * FROM alarm_model WHERE alarm_level='critical' AND status=false ORDER BY id DESC LIMIT 20", nativeQuery = true)
...
//	Select List of Latest 10 Alarms from database
	...
	@Query(value = "SELECT COUNT(d.alarm_level) FROM alarm_model d WHERE d.alarm_level = 'critical' AND d.status=false", nativeQuery = true)
